...
<p>Delivery: <strong><?= h($order['status'] === 'completed' ? 'Delivered' : ucfirst($order['status'])) ?></strong> &middot; Payment: <strong><?= h(ucwords(str_replace('_', ' ', $order['payment_status'] ?? 'unknown'))) ?></strong></p>
...
<li><?= h(ucfirst($event['event_type'])) ?>: <?= h($event['new_value'] === 'completed' ? 'Delivered' : ucwords(str_replace('_', ' ', $event['new_value']))) ?> &mdash; <?= h($event['created_at']) ?></li>
<?php endforeach; ?>
</ol>
</section>
