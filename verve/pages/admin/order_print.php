...
<header><h1 class="print-brand"><?php require __DIR__ . '/../../includes/brand-logo.php'; ?></h1><h2>Packing slip #<?= (int)$order['id'] ?></h2><p>Order placed: <?= h($order['created_at']) ?></p><p>Delivery: <?= h(ucfirst($order['status'])) ?> · Payment: <?= h(ucwords(str_replace('_',' ',$order['payment_status']))) ?></p></header>
